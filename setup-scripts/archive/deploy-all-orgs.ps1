...
Write-Host "Healthcare DLT Multi-Organization Chaincode Deployment" -ForegroundColor Cyan
...
"@
    ...
    $installResult = docker exec fabric-network-cli-1 bash -c $installCmd 2>&1
...
$createResult = docker exec fabric-network-cli-1 bash -c $createAdminCmd 2>&1
if ($LASTEXITCODE -eq 0) {
